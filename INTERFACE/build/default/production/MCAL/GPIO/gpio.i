...
Std_ReturnType pin_directoin_intialize(const pin_config *pin)
{
    ...
    else
    {
        ...
        {
            case INPUT:(*TRIS_array[pin->port]|=(1<<pin->pin));break;
            case OUTPUT:(*TRIS_array[pin->port]&=~(1<<pin->pin));break;
...
Std_ReturnType get_pin_directoin_status(const pin_config *pin,uint8 *status)
{
    ...
    {
        *status=((*TRIS_array[pin->port] >> pin->pin) & 0X01);
    }
    return ret;
}
# 61 "MCAL/GPIO/gpio.c"
Std_ReturnType pin_write_value(const pin_config *pin,uint8 value)
{
    ...
    else
    {
        switch(value)
        {
            case 1:(*LAT_array[pin->port]|=(1<<pin->pin)); break;
            case 0:(*LAT_array[pin->port]&=~(1<<pin->pin)); break;
            default :ret=(Std_ReturnType)0x00;
        }
    }
    return ret;
...
Std_ReturnType pin_read_value(const pin_config *pin,uint8 *value)
{
    ...
    {
        *value=((*PORT_array[pin->port] >> pin->pin) & 0X01);
    }
     return ret;
}
# 107 "MCAL/GPIO/gpio.c"
Std_ReturnType pin_toggle_value(const pin_config *pin)
{
     Std_ReturnType ret = (Std_ReturnType)0x01;
    if(((void*)0) == pin || pin->pin > 8 -1){
    ...
    {
        (*LAT_array[pin->port]^=(1<<pin->pin));
    }
     return ret;
}
# 127 "MCAL/GPIO/gpio.c"
Std_ReturnType gpio_port_direction_intialize(port_index port, uint8 direction)
{
    Std_ReturnType ret = (Std_ReturnType)0x01;
    if(port > 5 -1){
